...
| 企业内部系统接入环境 | 演示人员、实施人员 | 通过隔离的内部系统接入外壳，验证入站、出站和身份映射链路 |
...
企业内部系统接入环境不是动画播放器，也不能读取固定 JSON。它必须：
...
4. 将系统出站消息写入可查询的渠道 outbox，再由接入环境拉取并展示。
5. 明确显示“企业内部系统接入环境”，出站状态使用“已送达内部系统接入环境”，不得伪装成“生产外部渠道已送达”。
6. 生产外部渠道不在本项目执行范围，必须保持 `DISABLED_BY_POLICY`；所有外部渠道相关开发、演示和 UAT 进入企业内部系统接入环境。
...
```mermaid
flowchart LR
    A["企业内部系统接入环境（唯一外部渠道入口）"] --> C["Canonical Ingress Service"]
    X["生产外部渠道（策略禁用）"] -.-> Y["不初始化 / 不入队 / 不发送"]
    C --> D["Redis Streams"]
    D --> E["Queue Worker"]
    E --> F["Orchestrator"]
    F --> G["内部多 Agent"]
...
企业内部系统接入环境调用正式 Canonical Ingress 和业务服务。生产外部渠道 Adapter 在本项目中不可达，不得初始化或调用。
...
| 姓名 | 业务身份 | 系统角色 | 主要入口 |
|---|---|---|---|
| 李明 | 东门运营员、首次上报人 | `operator` | 企业内部系统接入环境 |
| 陈雨 | 设备检修员 | `operator` | 企业内部系统接入环境、员工任务页 |
| 王芳 | 当日值班经理 | `manager` | 管理后台、企业内部系统接入环境 |
| 张建国 | 资深设备主管、经验来源专家 | `operator` + 专家授权 | 企业内部系统接入环境 |
| 赵敏 | 组织知识负责人 | `manager` 或独立知识角色 | 管理后台 |
| 周琪 | 次日早班运营员 | `operator` | 企业内部系统接入环境 |
...
- 企业内部系统接入环境仅在 UAT 环境或授权角色中启用，并有醒目的接入环境标识。
...
**输入：** 使用管理员账号登录，打开接入渠道和运行诊断，执行一次只读健康检查。  
**动作：** 检查 App、PostgreSQL、Redis、ChromaDB、Queue Worker、8 个 Agent 和 DeepSeek；确认企业内部系统接入环境与生产外部渠道使用不同的就绪状态。
**可见结果：** 页面以人类可读名称显示各依赖健康状态；模型显示 `deepseek-v4-flash` 且最近真实调用成功；接入环境显示 `SIMULATOR_READY`。生产外部渠道必须显示“已按策略禁用（`DISABLED_BY_POLICY`）”，不能显示 `READY`。
**持久化证据：** 脱敏后的配置快照、健康检查记录、Agent 注册快照、最近 DeepSeek 调用记录、渠道就绪判定和管理员查看审计；任何证据均不得包含 API Key、生产渠道 Secret 或鉴权头。  
**失败路径：** 任一必需依赖不健康、DeepSeek 最近真实调用失败或接入身份主数据不完整时，主旅程不得开始；页面必须给出影响范围和修复入口。  
**验收断言：** 演示人员能在一个页面准确说明“哪些能力已就绪、哪些生产外部集成已禁用”；接入环境就绪不会把生产外部渠道状态改成 `READY`。

### E2E-01 打开企业运营助手并确认身份

**角色：** 李明。  
**界面：** `/simulator/wecom/`，内部系统接入会话列表与“悦山企业运营助手”聊天窗口。
**输入：** 由授权演示人员选择“李明 / 东门运营员 / 悦山景区”；生产员工不需要选择身份。  
**动作：** 打开助手会话。  
**可见结果：** 顶部显示李明的姓名、岗位和所属景区；历史会话从服务端加载；页面显示“企业内部系统接入环境”，不显示 Agent 选择器。
**持久化证据：** `users`、渠道身份映射表或等价关系、`sessions`、登录与身份解析 `audit_logs`。  
**失败路径：** 未绑定、已停用或跨场地的接入身份被拒绝，页面提示联系管理员绑定，不回退为匿名用户。  
...
**角色：** 系统后台处理，李明观察结果。  
**界面：** 企业内部系统接入环境消息状态与统一助手回复卡片；管理后台技术详情可查看处理阶段。
...
**角色：** 王芳查看，系统生成。  
**界面：** 管理后台事件详情中的“处置任务”区域；李明的内部系统接入会话收到摘要。  
...
### E2E-09 批准动作并送达到企业内部系统接入环境

**角色：** 王芳。  
**界面：** 管理后台审批详情、相关员工内部系统接入会话。
**输入：** 批准意见：“同意 12 号车停运观察，启用 7 号备用车，按调整后计划发车。”  
**动作：** 批准第二张审批；工具执行器写入渠道 outbox；企业内部系统接入环境读取出站消息。  
**可见结果：** 审批显示“已批准 / 已执行”；李明、陈雨和调度角色在企业内部系统接入环境实际收到通知卡，内容包含事件编号、车辆调整和负责人。
...
**验收断言：** 接入环境状态明确为“已送达内部系统接入环境”，不能写成“生产外部渠道已送达”；每个收件人只有一条出站通知。
...
**角色：** 张建国。  
**界面：** 企业内部系统接入环境中的“悦山企业运营助手”，不是管理后台 Persona 聊天弹窗。
...
**角色：** 赵敏、张建国。  
**界面：** 管理后台“经验资产 / 待审核”；张建国通过企业运营助手补充说明。  
...
**角色：** 周琪、刘海。  
**界面：** 次日早班企业内部系统接入环境统一助手会话；管理后台“审计与 Trace”。
**输入：**
...
| ID | 失败或恢复场景 | 预期结果 |
|---|---|---|
| UAT-F01 | 相同接入渠道 `MsgId` 或客户端幂等键重复投递 | 只产生一条消息 run、一个事件和一组任务；重复请求返回原结果 |
...
| UAT-F12 | 配置完整生产外部渠道凭据、历史送达记录或 API 身份后尝试启用或发送 | 渠道仍保持 `DISABLED_BY_POLICY`；不初始化客户端、不入队、不产生真实发送或假成功 |
| UAT-F13 | 接入环境身份无映射、已停用或无法确定租户 | 接入请求在业务入队前被拒绝并审计；不创建匿名会话、事件或空租户数据 |
...
7. 截图需覆盖 1440x900 桌面管理后台、企业内部系统接入环境桌面视图和 375x812 员工移动视图。
...
9. 企业内部系统接入环境真实贯通系统，但没有被错误宣传为生产外部渠道联调通过。
