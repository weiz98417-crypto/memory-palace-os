...
> 文档类型：新增产品设计文档  
> 文档边界：定义员工助手、企微模拟器和管理后台的页面结构、角色权限、跳转关系与展示规范。  
> 重要原则：管理后台不是员工 Agent 入口；企微模拟器不是动画页；三个入口共享同一套业务 API、Canonical Ingress、Agent、数据库和审计链路。  
...
| 企微模拟器 | `/simulator/wecom/` | 演示人员、经理、管理员 | 模拟员工在企微中使用助手，并展示与管理后台的实时互通 | `[新增]` |
| 管理后台 | `/admin/` | 经理、知识负责人、管理员 | 事件处置、任务审批、知识治理、经验发布、渠道和系统管理 | `[复用改造]` |

真实企微中的机器人本身不是一个 Web 页面。`/assistant/` 是 Web 备用员工入口，也是企微消息卡片的统一渲染基础；`/simulator/wecom/` 仅模拟企微渠道体验，但必须调用真实后端链路。
...
flowchart TD
    ...
    Identity --> Simulator["企微模拟器 /simulator/wecom/"]
...
| EA-06 | 我的 `/assistant/me` | 姓名、部门、岗位、所属场地、角色、企微绑定状态、通知偏好、隐私与授权说明 | 查看身份、退出登录、管理本人通知和经验授权 | `[新增]` |
...
| 页面与路由 | 核心组件 | 说明 | 状态 |
|---|---|---|---|
| 企微模拟器 `/simulator/wecom/` | 企业与场地选择、员工身份选择、会话选择、仿企微聊天区域、消息卡片、输入与附件、后台状态侧栏、证据抽屉 | 使用真实员工、真实会话和真实业务数据，不允许前端定时播放预设结果 | `[新增]` |
| 模拟会话 `/simulator/wecom/session/:id` | 当前员工、所属部门、机器人名称、完整消息历史、发送和回执状态 | 刷新页面后恢复同一真实会话 | `[新增]` |

### 5.2 模拟器三层界面

1. **员工可见层**  
   只显示仿企微对话、业务卡片、任务操作和引用依据，不显示 Agent 名称、UUID 或原始 JSON。

2. **演示控制层**  
   允许有权限的人员选择“悦山景区 / 东门运营员李明”等人类可读身份、新建演示会话和切换业务场景；不得允许任意伪造租户或越权身份。

3. **真实证据层**  
   默认折叠，展示渠道类型、受理时间、消息状态、DeepSeek 调用结果、业务资源链接和 Trace 链接。页面必须明确标注“企微接入模拟环境”，不得冒充真实企微已经完成认证。
...
- 模拟器证据抽屉可跳转到事件、会话、任务、审批和 Trace 详情。
- 后台渠道接入页可一键打开模拟器，但不得复制一套独立业务数据。
...
| AD-07 | 渠道接入 `/admin/channels` | 企微应用状态、身份映射、回调状态、最近收发记录、配置缺项、打开模拟器 | `[新增]`，复用现有企微网关与集成状态 |
...
| 编号 | 页面与路由 | 核心组件 | 复用关系 |
|---|---|---|---|
| AD-14 | 组织、人员与权限 `/admin/organization` | 企业、场地、部门、员工、角色、专家标记、企微身份映射、启停状态 | `[复用改造]` 现有 `management` |
...
| 角色 | 定位 | 默认入口 |
|---|---|---|
| 一线员工 / 现场操作员 | 使用助手上报、咨询和完成本人工作 | 员工助手或企微 |
...
| 模拟器消息 | 点击业务资源 | 员工视图详情；有后台权限时可继续进入后台详情 |
| 模拟器证据抽屉 | 点击 Trace 或资源 | 管理后台对应详情 |
...
| 渠道接入 | 点击“打开模拟器” | 带当前企业和渠道上下文进入企微模拟器 |
...
| 企微模拟器 | “真实链路证据”抽屉默认折叠 | 有模拟器权限的经理、管理员 |
| 管理后台业务页 | “技术与审计信息”默认折叠 | 经理可看业务 Trace 摘要；管理员可看完整脱敏信息 |
| 运维诊断 | 默认展开 | 系统管理员 |

### 10.3 永不显示的内容

- DeepSeek API Key、企微 Secret、Token、Cookie 和密码。
- 完整鉴权头、完整外部供应商请求体。
- 未脱敏的手机号、身份证号或其他敏感字段。
...
| 企微模拟器 | 完整控制台 1024px 至 1920px；聊天面板 360px 起 | 1024×768、1366×768、1440×900、1920×1080 | 大屏三栏；窄屏按“控制—聊天—证据”纵向排列；聊天始终可操作 |
...
- 员工助手、企微模拟器和管理后台引用的是同一业务资源，不创建三套数据。
